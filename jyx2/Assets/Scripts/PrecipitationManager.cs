...
public class PrecipitationManager : MonoBehaviour
{
    ...
    static Material CreateMaterialIfNull(Shader shader, ref Material reference) {
        if (reference == null) {
            reference = new Material(shader);
            reference.hideFlags = HideFlags.HideAndDontSave;
            reference.renderQueue = 3000;
            reference.enableInstancing = true;
        }
    ...
    void Update()
    {
        ...
        */
        ...
        RenderEnvironmentParticles(
            rain, CreateMaterialIfNull("Hidden/Environment/Rain", ref rainMaterial),
            maxTravelDistance, windRotationMatrix);
        RenderEnvironmentParticles(
            snow, CreateMaterialIfNull("Hidden/Environment/Snow", ref snowMaterial),
            maxTravelDistance, windRotationMatrix);
    }
